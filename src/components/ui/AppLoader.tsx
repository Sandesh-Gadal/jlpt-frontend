  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative', zIndex: 1 }}>
        ...
        {/* Torii gate SVG mark */}
        <div style={{ marginBottom: 36, animation: 'loaderFadeIn .7s ease both' }}>
        ...
        <div style={{ position: 'relative', width: 120, height: 120, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 28 }}>
          ...
          <div style={{ position: 'absolute', inset: 0, borderRadius: '50%', border: '1px solid rgba(110,170,84,0.12)', animation: 'loaderSpin 8s linear infinite' }}>
            <div style={{ position: 'absolute', width: 4, height: 4, borderRadius: '50%', background: 'var(--accent-primary)', top: 2, left: '50%', transform: 'translateX(-50%)', boxShadow: '0 0 6px var(--accent-primary)' }} />
          </div>
          {/* Mid ring dashed */}
      ...
      <style>{`
        ...
        @keyframes loaderDot     { 0%,100%{background:rgba(110,170,84,0.15)} 50%{background:var(--accent-primary);box-shadow:0 0 6px rgba(110,170,84,.5)} }
      `}</style>
    </div>
  );
}